...
use bevy::{prelude::*, render::options::{WgpuOptions, Backends}};
...
impl State {
    ...
    pub fn event_step(&mut self, passed_time: f64) {
...
fn event_handler_system(time: Res<Time>, mut state: ResMut<State>) {
    let passed_time = time.seconds_since_startup();
    state.event_step(passed_time);
}
...
fn render_event_system(state: Res<State>, mut query: Query<&mut Text, With<EventText>>) {
    let log = EVENTS.lock().expect("Failed to lock EVENTS mutex");
    let event_idx = state.event_idx;
    let last_event = &log[event_idx - 1];
    for mut text in query.iter_mut() {
        text.sections[1].value = format!("{} - {}", event_idx - 1, last_event.name());
    }
}
